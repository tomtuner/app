...
use ArtRequestORM\FlyerSizeQuery;

/**
 * Base class that represents a row from the 'flyer_size' table.
 *
 *
 *
 * @package    propel.generator.ArtRequest.om
 */
abstract class BaseFlyerSize extends BaseObject implements Persistent
{
    /**
     * Peer class name
     */
    const PEER = 'ArtRequestORM\\FlyerSizePeer';
    ...
    /**
     * The value for the flyer_size_type field.
     * @var        string
     */
    protected $flyer_size_type;
    ...
    /**
     * Get the [flyer_size_type] column value.
     *
     * @return string
     */
    public function getFlyerSizeType()
    {
        return $this->flyer_size_type;
    }
     ...
     * @return FlyerSize The current object (for fluent API support)
     */
    public function setFlyerSizeId($v)
    {
        ...
        if ($this->flyer_size_id !== $v) {
            ...
            $this->modifiedColumns[] = FlyerSizePeer::FLYER_SIZE_ID;
        }
    ...
    /**
     * Set the value of [flyer_size_type] column.
     *
     * @param string $v new value
     * @return FlyerSize The current object (for fluent API support)
     */
    public function setFlyerSizeType($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->flyer_size_type !== $v) {
            $this->flyer_size_type = $v;
            $this->modifiedColumns[] = FlyerSizePeer::FLYER_SIZE_TYPE;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->flyer_size_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
            $this->flyer_size_type = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = FlyerSizePeer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating FlyerSize object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = FlyerSizePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = FlyerSizeQuery::create()
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                FlyerSizePeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        $this->modifiedColumns[] = FlyerSizePeer::FLYER_SIZE_ID;
        if (null !== $this->flyer_size_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . FlyerSizePeer::FLYER_SIZE_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(FlyerSizePeer::FLYER_SIZE_ID)) {
            $modifiedColumns[':p' . $index++]  = '`FLYER_SIZE_ID`';
        }
        if ($this->isColumnModified(FlyerSizePeer::FLYER_SIZE_TYPE)) {
            $modifiedColumns[':p' . $index++]  = '`FLYER_SIZE_TYPE`';
        }

        $sql = sprintf(
            'INSERT INTO `flyer_size` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`FLYER_SIZE_ID`':
                        ...
                        break;
                    case '`FLYER_SIZE_TYPE`':
                        $stmt->bindValue($identifier, $this->flyer_size_type, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = FlyerSizePeer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = FlyerSizePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['FlyerSize'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['FlyerSize'][$this->getPrimaryKey()] = true;
        $keys = FlyerSizePeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getFlyerSizeId(),
            $keys[1] => $this->getFlyerSizeType(),
        );
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) $this->setFlyerSizeId($arr[$keys[0]]);
        if (array_key_exists($keys[1], $arr)) $this->setFlyerSizeType($arr[$keys[1]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(FlyerSizePeer::FLYER_SIZE_ID)) $criteria->add(FlyerSizePeer::FLYER_SIZE_ID, $this->flyer_size_id);
        if ($this->isColumnModified(FlyerSizePeer::FLYER_SIZE_TYPE)) $criteria->add(FlyerSizePeer::FLYER_SIZE_TYPE, $this->flyer_size_type);
    ...
    {
        $criteria = new Criteria(FlyerSizePeer::DATABASE_NAME);
        $criteria->add(FlyerSizePeer::FLYER_SIZE_ID, $this->flyer_size_id);
    ...
    /**
     * Generic method to set the primary key (flyer_size_id column).
     *
    ...
    public function isPrimaryKeyNull()
    {
        ...
        return null === $this->getFlyerSizeId();
    }
     ...
     * objects.
     *
     * @param object $copyObj An object of FlyerSize (or compatible) type.
    ...
    {
        $copyObj->setFlyerSizeType($this->getFlyerSizeType());
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setFlyerSizeId(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return FlyerSize Clone of current object.
     * @throws PropelException
     */
     ...
     *
     * @return FlyerSizePeer
     */
    public function getPeer()
    {
        if (self::$peer === null) {
            self::$peer = new FlyerSizePeer();
        }
     ...
     * If this FlyerSize is new, it will return
    ...
    public function setFlyerArtRequests(PropelCollection $flyerArtRequests, PropelPDO $con = null)
    {
        ...
        foreach ($this->flyerArtRequestsScheduledForDeletion as $flyerArtRequestRemoved) {
            $flyerArtRequestRemoved->setFlyerSize(null);
        }
    ...
    protected function doAddFlyerArtRequest($flyerArtRequest)
    {
        ...
        $flyerArtRequest->setFlyerSize($this);
    }
    ...
    public function removeFlyerArtRequest($flyerArtRequest)
    {
        if ($this->getFlyerArtRequests()->contains($flyerArtRequest)) {
            ...
            $flyerArtRequest->setFlyerSize(null);
        }
    }
     ...
     * Otherwise if this FlyerSize is new, it will return
     * an empty collection; or if this FlyerSize has previously
     * been saved, it will retrieve related FlyerArtRequests from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in FlyerSize.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
    ...
    {
        return (string) $this->exportTo(FlyerSizePeer::DEFAULT_STRING_FORMAT);
    }
