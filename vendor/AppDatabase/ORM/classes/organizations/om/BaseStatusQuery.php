...
use \Propel;
use \PropelPDO;
use OrganizationsORM\Status;
use OrganizationsORM\StatusPeer;
use OrganizationsORM\StatusQuery;
...
abstract class BaseStatusQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'organizations', $modelName = 'OrganizationsORM\\Status', $modelAlias = null)
	{
	...
	{
		$sql = 'SELECT `ID`, `STATUS` FROM `status` WHERE `ID` = :p0';
		try {
	...
	public function filterById($id = null, $comparison = null)
	{
		if (is_array($id)) {
			$useMinMax = false;
			if (isset($id['min'])) {
				$this->addUsingAlias(StatusPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
				$useMinMax = true;
			}
			if (isset($id['max'])) {
				$this->addUsingAlias(StatusPeer::ID, $id['max'], Criteria::LESS_EQUAL);
				$useMinMax = true;
			}
			if ($useMinMax) {
				return $this;
