...
/**
 * Base class that represents a row from the 'rit_college' table.
 *
 * 
 *
 * @package    propel.generator.commuterweek.om
 */
abstract class BaseRitCollege extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'CommuterWeekORM\\RitCollegePeer';
	...
	/**
	 * The value for the rit_college_name field.
	 * @var        string
	 */
	protected $rit_college_name;
	...
	/**
	 * Get the [rit_college_name] column value.
	 * 
	 * @return     string
	 */
	public function getRitCollegeName()
	{
		return $this->rit_college_name;
	}
	 ...
	 * @return     RitCollege The current object (for fluent API support)
	 */
	public function setRitCollegeId($v)
	{
		...
		if ($this->rit_college_id !== $v) {
			...
			$this->modifiedColumns[] = RitCollegePeer::RIT_COLLEGE_ID;
		}
	...
	/**
	 * Set the value of [rit_college_name] column.
	 * 
	 * @param      string $v new value
	 * @return     RitCollege The current object (for fluent API support)
	 */
	public function setRitCollegeName($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->rit_college_name !== $v) {
			$this->rit_college_name = $v;
			$this->modifiedColumns[] = RitCollegePeer::RIT_COLLEGE_NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rit_college_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->rit_college_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RitCollegePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RitCollege object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RitCollegePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RitCollegeQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				RitCollegePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = RitCollegePeer::RIT_COLLEGE_ID;
		if (null !== $this->rit_college_id) {
			throw new PropelException('Cannot insert a value for auto-increment primary key (' . RitCollegePeer::RIT_COLLEGE_ID . ')');
		}

		 // check the columns in natural order for more readable SQL queries
		if ($this->isColumnModified(RitCollegePeer::RIT_COLLEGE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`RIT_COLLEGE_ID`';
		}
		if ($this->isColumnModified(RitCollegePeer::RIT_COLLEGE_NAME)) {
			$modifiedColumns[':p' . $index++]  = '`RIT_COLLEGE_NAME`';
		}

		$sql = sprintf(
			'INSERT INTO `rit_college` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`RIT_COLLEGE_ID`':
						...
						break;
					case '`RIT_COLLEGE_NAME`':
						$stmt->bindValue($identifier, $this->rit_college_name, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RitCollegePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RitCollegePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RitCollege'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RitCollege'][$this->getPrimaryKey()] = true;
		$keys = RitCollegePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getRitCollegeId(),
			$keys[1] => $this->getRitCollegeName(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setRitCollegeId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setRitCollegeName($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RitCollegePeer::RIT_COLLEGE_ID)) $criteria->add(RitCollegePeer::RIT_COLLEGE_ID, $this->rit_college_id);
		if ($this->isColumnModified(RitCollegePeer::RIT_COLLEGE_NAME)) $criteria->add(RitCollegePeer::RIT_COLLEGE_NAME, $this->rit_college_name);
	...
	{
		$criteria = new Criteria(RitCollegePeer::DATABASE_NAME);
		$criteria->add(RitCollegePeer::RIT_COLLEGE_ID, $this->rit_college_id);
	...
	/**
	 * Generic method to set the primary key (rit_college_id column).
	 *
	...
	{
		return null === $this->getRitCollegeId();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RitCollege (or compatible) type.
	...
	{
		$copyObj->setRitCollegeName($this->getRitCollegeName());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setRitCollegeId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RitCollege Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RitCollegePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RitCollegePeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RitCollege is new, it will return
	 ...
	 * Otherwise if this RitCollege is new, it will return
	 * an empty collection; or if this RitCollege has previously
	 * been saved, it will retrieve related Commuters from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RitCollege.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(RitCollegePeer::DEFAULT_STRING_FORMAT);
	}
