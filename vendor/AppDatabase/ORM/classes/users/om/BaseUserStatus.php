...
abstract class BaseUserStatus extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'UsersORM\\UserStatusPeer';
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = UserStatusPeer::USER_ID;
		}
	...
	public function setStatusId($v)
	{
		...
		if ($this->status_id !== $v) {
			$this->status_id = $v;
			$this->modifiedColumns[] = UserStatusPeer::STATUS_ID;
		}
	...
	public function setVerified($v)
	{
		...
		if ($this->verified !== $v) {
			$this->verified = $v;
			$this->modifiedColumns[] = UserStatusPeer::VERIFIED;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->verified = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = UserStatusPeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(UserStatusPeer::USER_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USER_ID`';
		}
		if ($this->isColumnModified(UserStatusPeer::STATUS_ID)) {
			$modifiedColumns[':p' . $index++]  = '`STATUS_ID`';
		}
		if ($this->isColumnModified(UserStatusPeer::VERIFIED)) {
	...
	{
		if (isset($alreadyDumpedObjects['UserStatus'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['UserStatus'][serialize($this->getPrimaryKey())] = true;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UserStatusPeer::USER_ID)) $criteria->add(UserStatusPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(UserStatusPeer::STATUS_ID)) $criteria->add(UserStatusPeer::STATUS_ID, $this->status_id);
		if ($this->isColumnModified(UserStatusPeer::VERIFIED)) $criteria->add(UserStatusPeer::VERIFIED, $this->verified);
	...
	{
		$criteria = new Criteria(UserStatusPeer::DATABASE_NAME);
		$criteria->add(UserStatusPeer::USER_ID, $this->user_id);
		$criteria->add(UserStatusPeer::STATUS_ID, $this->status_id);
	...
	{
		return (null === $this->getUserId()) && (null === $this->getStatusId());
	}
