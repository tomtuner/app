...
use ClubsORM\QuarterlyEventsQuery;
 ...
 * @method     QuarterlyEventsQuery orderByQuarterlyReportId($order = Criteria::ASC) Order by the quarterly_report_id column
 * @method     QuarterlyEventsQuery orderByQuarterlyEventTypeId($order = Criteria::ASC) Order by the quarterly_event_type_id column
 ...
 * @method     QuarterlyEventsQuery groupByQuarterlyReportId() Group by the quarterly_report_id column
 * @method     QuarterlyEventsQuery groupByQuarterlyEventTypeId() Group by the quarterly_event_type_id column
 ...
 * @method     QuarterlyEvents findOneByQuarterlyReportId(int $quarterly_report_id) Return the first QuarterlyEvents filtered by the quarterly_report_id column
 * @method     QuarterlyEvents findOneByQuarterlyEventTypeId(int $quarterly_event_type_id) Return the first QuarterlyEvents filtered by the quarterly_event_type_id column
 ...
 * @method     array findByQuarterlyReportId(int $quarterly_report_id) Return QuarterlyEvents objects filtered by the quarterly_report_id column
 * @method     array findByQuarterlyEventTypeId(int $quarterly_event_type_id) Return QuarterlyEvents objects filtered by the quarterly_event_type_id column
...
abstract class BaseQuarterlyEventsQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'clubs', $modelName = 'ClubsORM\\QuarterlyEvents', $modelAlias = null)
	{
	...
	{
		$sql = 'SELECT `ID`, `QUARTERLY_REPORT_ID`, `QUARTERLY_EVENT_TYPE_ID`, `HELD_ON`, `NUM_MEMBERS`, `NUM_OUTSIDE`, `DESCRIPTION` FROM `quarterly_events` WHERE `ID` = :p0';
		try {
	...
	public function filterByQuarterlyReportId($quarterlyReportId = null, $comparison = null)
	{
		if (is_array($quarterlyReportId)) {
			...
			if (isset($quarterlyReportId['min'])) {
				$this->addUsingAlias(QuarterlyEventsPeer::QUARTERLY_REPORT_ID, $quarterlyReportId['min'], Criteria::GREATER_EQUAL);
				$useMinMax = true;
			}
			if (isset($quarterlyReportId['max'])) {
				$this->addUsingAlias(QuarterlyEventsPeer::QUARTERLY_REPORT_ID, $quarterlyReportId['max'], Criteria::LESS_EQUAL);
				$useMinMax = true;
			}
			if ($useMinMax) {
				return $this;
		...
		return $this->addUsingAlias(QuarterlyEventsPeer::QUARTERLY_REPORT_ID, $quarterlyReportId, $comparison);
	}

	/**
	 * Filter the query on the quarterly_event_type_id column
	 *
	 * Example usage:
	 * <code>
	 * $query->filterByQuarterlyEventTypeId(1234); // WHERE quarterly_event_type_id = 1234
	 * $query->filterByQuarterlyEventTypeId(array(12, 34)); // WHERE quarterly_event_type_id IN (12, 34)
	 * $query->filterByQuarterlyEventTypeId(array('min' => 12)); // WHERE quarterly_event_type_id > 12
	 * </code>
	 *
	 * @param     mixed $quarterlyEventTypeId The value to use as filter.
	 *              Use scalar values for equality.
	 *              Use array values for in_array() equivalent.
	 *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 ...
	 * @return    QuarterlyEventsQuery The current query, for fluid interface
	 */
	public function filterByQuarterlyEventTypeId($quarterlyEventTypeId = null, $comparison = null)
	{
		if (is_array($quarterlyEventTypeId)) {
			$useMinMax = false;
			if (isset($quarterlyEventTypeId['min'])) {
				$this->addUsingAlias(QuarterlyEventsPeer::QUARTERLY_EVENT_TYPE_ID, $quarterlyEventTypeId['min'], Criteria::GREATER_EQUAL);
				$useMinMax = true;
			}
			if (isset($quarterlyEventTypeId['max'])) {
				$this->addUsingAlias(QuarterlyEventsPeer::QUARTERLY_EVENT_TYPE_ID, $quarterlyEventTypeId['max'], Criteria::LESS_EQUAL);
				$useMinMax = true;
			}
			if ($useMinMax) {
				return $this;
		...
		return $this->addUsingAlias(QuarterlyEventsPeer::QUARTERLY_EVENT_TYPE_ID, $quarterlyEventTypeId, $comparison);
	}
