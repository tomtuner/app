...
abstract class BaseLog extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'NewClubsORM\\LogPeer';
	...
	public function setUserid($v)
	{
		...
		if ($this->userid !== $v) {
			$this->userid = $v;
			$this->modifiedColumns[] = LogPeer::USERID;
		}
	...
	public function setMode($v)
	{
		...
		if ($this->mode !== $v) {
			$this->mode = $v;
			$this->modifiedColumns[] = LogPeer::MODE;
		}
	...
	public function setPage($v)
	{
		...
		if ($this->page !== $v) {
			$this->page = $v;
			$this->modifiedColumns[] = LogPeer::PAGE;
		}
	...
	public function setTime($v)
	{
		...
		if ($this->time !== null || $dt !== null) {
			...
			if ( ($currentDateAsString !== $newDateAsString) // normalized values don't match
				...
				$this->modifiedColumns[] = LogPeer::TIME;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 7; // 7 = LogPeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(LogPeer::USERID)) {
			$modifiedColumns[':p' . $index++]  = '`USERID`';
		}
		if ($this->isColumnModified(LogPeer::MODE)) {
			$modifiedColumns[':p' . $index++]  = '`MODE`';
		}
		if ($this->isColumnModified(LogPeer::PAGE)) {
			$modifiedColumns[':p' . $index++]  = '`PAGE`';
		}
		if ($this->isColumnModified(LogPeer::DATA)) {
			$modifiedColumns[':p' . $index++]  = '`DATA`';
		}
		if ($this->isColumnModified(LogPeer::DATE)) {
			$modifiedColumns[':p' . $index++]  = '`DATE`';
		}
		if ($this->isColumnModified(LogPeer::TIME)) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LogPeer::USERID)) $criteria->add(LogPeer::USERID, $this->userid);
		if ($this->isColumnModified(LogPeer::MODE)) $criteria->add(LogPeer::MODE, $this->mode);
		if ($this->isColumnModified(LogPeer::PAGE)) $criteria->add(LogPeer::PAGE, $this->page);
		...
		if ($this->isColumnModified(LogPeer::TIME)) $criteria->add(LogPeer::TIME, $this->time);
