 ...
 * @method     QuarterlyUpcomingEventsQuery orderByQuarterlyDataId($order = Criteria::ASC) Order by the quarterly_data_id column
 ...
 * @method     QuarterlyUpcomingEventsQuery groupByQuarterlyDataId() Group by the quarterly_data_id column
 ...
 * @method     QuarterlyUpcomingEvents findOneByQuarterlyDataId(int $quarterly_data_id) Return the first QuarterlyUpcomingEvents filtered by the quarterly_data_id column
 ...
 * @method     array findByQuarterlyDataId(int $quarterly_data_id) Return QuarterlyUpcomingEvents objects filtered by the quarterly_data_id column
...
abstract class BaseQuarterlyUpcomingEventsQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'newclubs', $modelName = 'NewClubsORM\\QuarterlyUpcomingEvents', $modelAlias = null)
	{
	...
	{
		$sql = 'SELECT `ID`, `QUARTERLY_DATA_ID`, `HELD_ON`, `LOCATION`, `DESCRIPTION`, `CONTACT` FROM `quarterly_upcoming_events` WHERE `ID` = :p0';
		try {
	...
	public function filterByQuarterlyDataId($quarterlyDataId = null, $comparison = null)
	{
		if (is_array($quarterlyDataId)) {
			...
			if (isset($quarterlyDataId['min'])) {
				$this->addUsingAlias(QuarterlyUpcomingEventsPeer::QUARTERLY_DATA_ID, $quarterlyDataId['min'], Criteria::GREATER_EQUAL);
				$useMinMax = true;
			}
			if (isset($quarterlyDataId['max'])) {
				$this->addUsingAlias(QuarterlyUpcomingEventsPeer::QUARTERLY_DATA_ID, $quarterlyDataId['max'], Criteria::LESS_EQUAL);
				$useMinMax = true;
			}
			if ($useMinMax) {
				return $this;
		...
		return $this->addUsingAlias(QuarterlyUpcomingEventsPeer::QUARTERLY_DATA_ID, $quarterlyDataId, $comparison);
	}
