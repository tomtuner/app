...
abstract class BaseUserProfile extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'NewClubsORM\\UserProfilePeer';
	...
	/**
	 * The value for the preffered_email field.
	 * Note: this column has a database default value of: ''
	 * @var        string
	 */
	protected $preffered_email;
	...
	/**
	 * Get the [preffered_email] column value.
	 * 
	 * @return     string
	 */
	public function getPrefferedEmail()
	{
		return $this->preffered_email;
	}
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = UserProfilePeer::TYPE;
		}
	...
	public function setSendEmail($v)
	{
		...
		if ($this->send_email !== $v) {
			$this->send_email = $v;
			$this->modifiedColumns[] = UserProfilePeer::SEND_EMAIL;
		}
	...
	/**
	 * Set the value of [preffered_email] column.
	 * 
	 * @param      string $v new value
	 * @return     UserProfile The current object (for fluent API support)
	 */
	public function setPrefferedEmail($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->preffered_email !== $v) {
			$this->preffered_email = $v;
			$this->modifiedColumns[] = UserProfilePeer::PREFFERED_EMAIL;
		}
	...
	public function setHomePhone($v)
	{
		...
		if ($this->home_phone !== $v) {
			$this->home_phone = $v;
			$this->modifiedColumns[] = UserProfilePeer::HOME_PHONE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->send_email = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->preffered_email = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->home_phone = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(UserProfilePeer::USER_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USER_ID`';
		}
		if ($this->isColumnModified(UserProfilePeer::TYPE)) {
			$modifiedColumns[':p' . $index++]  = '`TYPE`';
		}
		if ($this->isColumnModified(UserProfilePeer::SEND_EMAIL)) {
			$modifiedColumns[':p' . $index++]  = '`SEND_EMAIL`';
		}
		if ($this->isColumnModified(UserProfilePeer::PREFFERED_EMAIL)) {
			$modifiedColumns[':p' . $index++]  = '`PREFFERED_EMAIL`';
		}
		if ($this->isColumnModified(UserProfilePeer::HOME_PHONE)) {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`SEND_EMAIL`':
						...
						break;
					case '`PREFFERED_EMAIL`':
						$stmt->bindValue($identifier, $this->preffered_email, PDO::PARAM_STR);
						break;
					case '`HOME_PHONE`':
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array())
	{
		...
		$result = array(
			...
			$keys[3] => $this->getPrefferedEmail(),
			$keys[4] => $this->getHomePhone(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setPrefferedEmail($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setHomePhone($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UserProfilePeer::TYPE)) $criteria->add(UserProfilePeer::TYPE, $this->type);
		if ($this->isColumnModified(UserProfilePeer::SEND_EMAIL)) $criteria->add(UserProfilePeer::SEND_EMAIL, $this->send_email);
		if ($this->isColumnModified(UserProfilePeer::PREFFERED_EMAIL)) $criteria->add(UserProfilePeer::PREFFERED_EMAIL, $this->preffered_email);
		if ($this->isColumnModified(UserProfilePeer::HOME_PHONE)) $criteria->add(UserProfilePeer::HOME_PHONE, $this->home_phone);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setPrefferedEmail($this->getPrefferedEmail());
		$copyObj->setHomePhone($this->getHomePhone());
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setUserId('0'); // this is a auto-increment column, so set to default value
		}
	}
